...
class ClaimsManager(private val context: Context) {
    ...
    fun getUserClaims(id: String): Flow<List<ClaimInformation>> {
        return context.dataStore.data.map { preferences ->
            ...
            (0 until numberOfClaims).map { index ->
                ClaimInformation(claimId = claimsIdsArray.getOrElse(index) { "" },
    ...
    suspend fun updateOrInsertClaimAtIndex(
        userId: String, index: Int, newClaim: ClaimInformation, isInsert: Boolean
    ) {
        context.dataStore.edit { preferences ->
            ...
            val claimsPhotoList = gson.fromJson(
                preferences[claimsPhotoKey(userId)] ?: "[]", Array<String>::class.java
            ).toMutableList()
            val claimsLocationList = gson.fromJson(
                preferences[claimsLocationKey(userId)] ?: "[]", Array<String>::class.java
            ).toMutableList()
            val claimsStatusList = gson.fromJson(
                preferences[claimsStatusKey(userId)] ?: "[]", Array<String>::class.java
            ).toMutableList()
